...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i<=num_rolls:
        current=dice()
        if sum==1 or current==1:
            sum=1
            i+=1
            continue
        sum, i = sum + current,i+1
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    number1=player_score % 10
    number2=opponent_score // 10 % 10
    if (number1 - number2 == 0):
        return 1
    else:
        return 3 * abs (number1 - number2)
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    judge = make_averaged(roll_dice,samples_count)
    max=0
    point_i=1
    for i in range(1,11):
        n=judge(i,dice)
