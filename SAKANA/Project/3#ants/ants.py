...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if ((self.is_container==True and self.ant_contained==None) or (place.ant!=None and place.ant.is_container==True and place.ant.ant_contained==None)) and not (self.is_container==True and place.ant.is_container==True):
                ...
                assert False, "Two ants in {}".format(place)
            # END Problem 8b
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        route=self.place
        n=0
        while route.entrance!=None:
            if route.bees!=[] and self.lower_bound<=n<=self.upper_bound:
                return random_bee(route.bees)
            route = route.entrance
            n+=1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        index=0
        while i < n:
            Ant.reduce_health(route.bees[i-index],amount)
            if len(route.bees)!=len(sbees):
                sbees=route.bees
                index+=1
            i+=1
        ...
        if route.ant==None:
            ...
            n=len(sbees)
            index=0
            while i<n and route.bees!=[]:
                Ant.reduce_health(route.bees[i-index],self.damage)
                if len(route.bees)!=len(sbees):
                    sbees=route.bees[:]
                    index+=1
                i+=1
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if other.is_container != True and self.ant_contained==None:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        ThrowerAnt.throw_at(self,self.nearest_bee())
        route = self.place.exit
        while route!=None:
            if route.ant!=None:
                route.ant.double()
            route = route.exit
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        dic = {}
        while route!=None:
            if route.ant!=None and route.ant!=self:
                dic[route.ant] = distance
            if route.bees != []:
                for x in route.bees:
                    dic[x] = distance
            distance+=1
